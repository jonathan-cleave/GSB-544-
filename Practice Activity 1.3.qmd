...
The difference between the two plots is the first uses a dichotomous variable to group mpg for am vs non am cars, while the second combines am and non am cars across a range for mpg.
...
The diffrence between the string plot is it adds a color overlay of the different cylinders based on the hp, while converting the cylinders back to a float value removes the overlay, but doesn't change the histogram.
